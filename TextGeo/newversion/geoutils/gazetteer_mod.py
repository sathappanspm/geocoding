...
class GeoNames(BaseGazetteer):
    ...
    def get_locInfo(self, country=None, admin=None, city=None):
        ...
        countryInfo = self._querycountry(country) or altdbquery(country, olddb, featureCode='pcli')
        ...
        if admin not in (None, '', '-'):
            ...
            admins = self.db.query(admin,
                                countryCode=countrycode.lower(), featureCode=['adm1', 'adm1h'])
            ...
            if city not in (None, []):
                ...
                if pts == []:
                    if (adms[0] in ctys[0].alternatenames or admin in ctys[0].alternatenames):
                        pts = admins

                    elif ctys[0].name == city.lower() or ctys[0].asciiname == city.lower() or city.lower() in ctys[0].alternatenames:
                        pts = ctys
            else:
                pts = admins
