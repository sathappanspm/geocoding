...
GeocodingRegion = namedtuple("GeocodingRegion", "lac mena")
GEO_REGION = GeocodingRegion(lac='lac', mena='mena')

WG_VERSION = {GEO_REGION.lac: 'v3',
              GEO_REGION.mena: 'v1'}

LANDMARKS_VERSION = {GEO_REGION.lac: '1k',
                     GEO_REGION.mena: '0'}

DIRTY_LANDMARKS_VERSION = {GEO_REGION.lac: '4k',
                           GEO_REGION.mena: '0'}

WG_DATA = os.path.join(os.path.dirname(__file__), "world-gazetteer-%s.%s.txt")
# NOTE:lac_co_admin - contains info on all latin-american countries
CO_ADMIN_DATA = os.path.join(os.path.dirname(__file__), "%s_co_admin.txt")
# landmarks having high likelihood to specific locations in GSR data
LANDMARKS_DATA = os.path.join(os.path.dirname(__file__), "%s_landmarks_%s.txt")
# dirty landmarks having high likelihood to specific locations in GSR data
DIRTY_LANDMARKS_DATA = os.path.join(os.path.dirname(__file__), "%s_landmarks_%s.txt")
...
LAC_WG_FIELDS = ["id", "name", "alt_names", "orig_names", "type", "pop",
                 "latitude", "longitude",
...
PRIORITY_POLICY = ['tweet_text_country', 'geo_coordinates', 'geo_place', 'rt_user_location',
                   'user_location', 'tweet_text', 'landmarks',
                   'dirty_landmarks', 'rt_user_description', 'user_description']
...
def get_wg_data(wg_data, region):
    if wg_data == WG_DATA:
        if "__loader__" in globals():
            import cStringIO
            f = __loader__.get_data("embers/" + os.path.basename(WG_DATA % (region,
                                                                 WG_VERSION[region])))
            f = cStringIO.StringIO(f)
        else:
            f = open(WG_DATA % (region, WG_VERSION[region]))
    else:
        f = open(wg_data)
    return f


def get_co_admin_data(co_admin_data, region):
    if co_admin_data == CO_ADMIN_DATA:
        if "__loader__" in globals():
            import cStringIO
            f = __loader__.get_data("embers/" + os.path.basename(CO_ADMIN_DATA % region))
            f = cStringIO.StringIO(f)
        else:
            f = open(CO_ADMIN_DATA % region)
    else:
        f = open(co_admin_data)
    return f


def get_landmarks_data(landmarks_data, region):
    if landmarks_data == LANDMARKS_DATA:
        if "__loader__" in globals():
            import cStringIO
            f = __loader__.get_data("embers/" +
                                    os.path.basename(LANDMARKS_DATA % (region,
                                                                       LANDMARKS_VERSION[region])))
            f = cStringIO.StringIO(f)
        else:
            f = open(LANDMARKS_DATA % (region, LANDMARKS_VERSION[region]))
    else:
        f = open(landmarks_data)
    return f


def get_dirty_landmarks_data(dirty_landmarks_data, region):
    if dirty_landmarks_data == DIRTY_LANDMARKS_DATA:
        if "__loader__" in globals():
            import cStringIO
            dirty_landmarks_loc = DIRTY_LANDMARKS_DATA % (region,
                                                          DIRTY_LANDMARKS_VERSION[region])
            f = __loader__.get_data("embers/" +
                                    os.path.basename(dirty_landmarks_loc))
            f = cStringIO.StringIO(f)
        else:
            f = open(DIRTY_LANDMARKS_DATA % (region, DIRTY_LANDMARKS_VERSION[region]))
    else:
...
class Geo(object):
    ...
    """
    def __init__(self, geo_region=GEO_REGION.lac, wg_data=WG_DATA, co_admin_data=CO_ADMIN_DATA,
        ...
        self.__version__ = "{0}-{1}-{2}-{3}-{4}".format(
            ...
            __version__,
            hashlib.md5(get_wg_data(wg_data, self.geo_region).read()).hexdigest(),
            hashlib.md5(get_co_admin_data(co_admin_data, self.geo_region).read()).hexdigest(),
            hashlib.md5(" ".join(self.priority_policy)).hexdigest(),
            hashlib.md5(get_landmarks_data(landmarks_data, self.geo_region).read()).hexdigest())
        ...
        if self.geo_region == GEO_REGION.lac:
            self.nstr = nstr
        else:
        ...
        self.landmarks = {}
        try:
            f = get_landmarks_data(landmarks_data, self.geo_region)
            ...
            for r in reader:
                ...
                self.landmarks[self.nstr(r['name'])] = [object_or_none(r['city']),
                                                        r['country'],
                                                        object_or_none(r['admin1']),
                                                        None, None, 0,
                                                        float_or_none(r['latitude']),
                                                        float_or_none(r['longitude']),
                                                        int_or_none(r["id"]), 0,
                                                        r['name'],
                                                        float_or_none(r["location_score"])]
        ...
        if use_dirty_landmarks:
            try:
                f = get_dirty_landmarks_data(dirty_landmarks_data, self.geo_region)
                dialect = csv.Sniffer().sniff(f.read(10240), delimiters="\t")
                f.seek(0)
                reader = csv.DictReader(f, dialect=dialect,
                ...
                # TODO include aliases in file since we are
                # not lower casing keys
                self.dirty_landmarks[self.nstr(r['name'])] = [object_or_none(r['city']),
                                                              r['country'],
                                                              object_or_none(r['admin1']),
                                                              None, None, 0,
                                                              float_or_none(r['latitude']),
        ...
        try:
            f = get_co_admin_data(co_admin_data, self.geo_region)
            dialect = csv.Sniffer().sniff(f.read(10240), delimiters="\t")
            f.seek(0)
        ...
        f = get_wg_data(wg_data, self.geo_region)
        dialect = csv.Sniffer().sniff(f.read(10240), delimiters="\t")
        f.seek(0)
        if self.geo_region == GEO_REGION.lac:
            fieldnames = LAC_WG_FIELDS
        elif self.geo_region == GEO_REGION.mena:
            fieldnames = MENA_WG_FIELDS
        reader = csv.DictReader(f, dialect=dialect, quoting=csv.QUOTE_NONE, fieldnames=fieldnames)
        self.ci_aliases = {}
        ...
        for r in reader:
            ...
            ci_names = [alt_nm.strip() for alt_nm in r['alt_names'].split(',')
                        if len(alt_nm.strip()) > 0]
            ci_names.extend([orig_nm.strip() for orig_nm in r['orig_names'].split(',')
                             if len(orig_nm.strip()) > 0])
            for ci in ci_names:
    ...
    def get_all_countries(self):
        '''Returns all countries extracted from gazetteer
        :returns: list of country names
        '''
        # well, that was easy
        return self.co_names.keys()
        ...
        if ci in self.bguess:
            if co and co in self.bguess[ci]:
                ...
                if len(self.bguess[ci]) == 1:
                    ...
                    if a1:
                        possible_co = set([co_guess for co_guess in
                                          self.bguess[ci].keys()
                                          if a1 in self.bguess[ci][co_guess]])
    ...
    def lookup_capital_city(self, capital_city, country):
        if capital_city in self.co_capital_cities:
            ...
            return self.best_guess_city(capital_city, co)
        else:
            return None
    ...
    def lookup_city(self, lat1, lon1, maxd=30.):
        ...
        """
        i, d = self.kdtree.query([lat1, lon1], distance_upper_bound=maxd / 25)
        if i == np.inf:
            return None
    ...
    def geo_from_lat_lon(self, lat, lon):
        ...
        if lat is not None and lon is not None:
            city_t = self.lookup_city(lat, lon)
            if city_t:
                return city_t[0]
        return None
    ...
    def geo_normalize_gnip(tweet):
        ...
        texts['language'] = tweet.get('twitter_lang', '')
        return lat, lon, places, texts
    ...
    def _match_for_names(self, loc, city=None, admin1=None, country=None):
        if loc:
            ...
            #if 'distrito federal' in loc:
            #    admin1 = "distrito federal"
            #    loc = loc.replace(admin1, " ", 1)
            #if not city:
    ...
    def _apply_correction(self, geos):
        if self.geo_region == GEO_REGION.lac and geos and geos[1] and geos[1] == 'Mexico' and geos[2] and\
           geos[2] == "Distrito Federal":
            # output Ciudad de México as city
            return self.geo_from_lat_lon(19.43, -99.14)
    ...
    def _geo_normalize(self, lat, lon, places, texts, enr, policies, do_correction=True):
        ...
        :param do_correction: apply user-defined corrections for known payload issues
        :returns: (city, country, admin1,
        ...
        for policy in policies:
            if policy == 'tweet_text':
                if 'text' in texts and texts['text']:
                    geos = self._geos_from_tweet_text(texts['text'])
            elif policy == 'dirty_landmarks' and self.use_dirty_landmarks:
                if 'text' in texts and texts['text']:
                    geos = self.geo_from_landmarks(texts['text'], dirty=True)
            elif policy == 'landmarks':
                if 'text' in texts and texts['text']:
                    geos = self.geo_from_landmarks(texts['text'])
            elif policy == 'geo_place':
                ...
                if self.debug and not geos:
                    self.log.info("lat-lon couldn't be resolved {0}".format(
                                  (lat, lon)))
            elif policy == 'rt_user_location':
                if 'rt_user_location' in texts and texts['rt_user_location']:
                    geos = self.match_for_lat_lon(texts['rt_user_location'])
                    if not geos:
                        loc_str, ci, a, co = self.match_for_names(
                            texts['rt_user_location'])
                        loc_str, ci, a, co = self.match_for_code(
                            texts['rt_user_location'], ci, a, co)
                ...
                if 'RNT_Translation' in enr and enr['RNT_Translation']:
                    ...
                    candidate_locations += [nstr(l['translation']['name'], False) for
                                            l in locations if l['translation']['name']]
                    best_loc = None
            ...
            #            if not has_co:
            #                best_match, best_score, entity = self._best_match(locations,
            #                                                                  user_loc_str)
            #                if entity:
            #                    if entity['name'] in MENA_COUNTRIES:
            #                        geos = self.resolve(None, None, entity['name'],
            #                                            RESOLUTION_LEVEL.country)
            #                    elif best_match in MENA_COUNTRIES:
            #                        geos = self.resolve(None, None, best_match,
            #                                            RESOLUTION_LEVEL.country)
            #                    else:
            #                        coords = entity['coordinates']
            #                        geos = self.geo_from_lat_lon(coords['latitude'],
            #                                                     coords['longitude'], 110)
            #                    if geos:
            #                        best_policy.append(":".join([best_match, user_loc_str,
            #                                                     str(best_score)]))
            elif policy == 'rt_user_description':
                if 'rt_user_description' in texts and\
                   texts['rt_user_description']:
                    loc_str, ci, a, co = self.match_for_names(
                        texts['rt_user_description'])
                    #loc_str, ci, a, co = self.match_for_code(
                    #    texts['rt_user_description'], ci, a, co)
                    geos = self.resolve(ci, a, co, RESOLUTION_LEVEL.country)
            elif policy == 'user_description':
                if 'user_description' in texts and texts['user_description']:
                    loc_str, ci, a, co = self.match_for_names(
                        texts['user_description'])
                    #loc_str, ci, a, co = self.match_for_code(
                    #    texts['user_description'], ci, a, co)
                    geos = self.resolve(ci, a, co, RESOLUTION_LEVEL.country)
            # iterative improvement of geocode
            if geos:
                if best_geos:
        ...
        if best_geos is None:
            ...
            best_geos = self._apply_correction(best_geos) if do_correction else best_geos
            return best_geos + (",".join(best_policy),)

    def geo_normalize(self, tweet, policy=None):
        if "embersGeoCode" in tweet:
            if ("geocode_version" in tweet["embersGeoCode"] and
    ...
    def annotate(self, tweet, keyname='embersGeoCode', policy=None):
        ...
        """
        geos = self.geo_normalize(tweet, policy)
        tweet[keyname] = {
